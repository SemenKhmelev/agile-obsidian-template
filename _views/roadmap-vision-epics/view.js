console.log("\n#################################################\n" +
    "        START ROADMAP VISION EPICS VIEW        " +
    "\n#################################################\n");
...
if (!current) {
    ...
    console.warn("roadmap-vision-epics: current page is undefined");
    return;
}
...
if (!currentEntry || currentEntry.type !== "vision") {
    dv.paragraph("Отчет доступен только для vision-страниц");
    console.warn(`roadmap-vision-epics: page ${currentPath} не является vision`);
    return;
}
