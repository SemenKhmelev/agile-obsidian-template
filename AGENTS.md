...
В репозитории лежат два скилла. Тело каждого — в `.agents/skills/<имя>/SKILL.md`, рядом
симлинк `.claude/skills/<имя>`: Codex читает первый каталог, Claude Code — второй.

- **`obsidian-time-log`** — занесение и правка списаний времени за день: как определить
  дату и спринт, откуда восстановить состав дня, на какую карточку списывать, как свести
  сумму и поправить уже занесённый день.
- **`run-obsidian`** — запуск настоящего Obsidian на копии vault, скриншоты доски, ленты
  и отчётов, выполнение JS в окне приложения. Нужен, когда правка в `_scripts/*.js`,
  сниппетах или вьюхах должна быть проверена в приложении, а не в статичном макете.
  Копия снимается без `.git`, obsidian-git в ней выключен, поэтому ничего не уходит на сервер.
