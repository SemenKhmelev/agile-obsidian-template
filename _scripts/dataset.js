/**
 * Возвращает сырой денормализованный массив данных, где каждая строка является одним внесением времени из файла комментария.
 * @param {*} dvarg передать актуальный dv
 * @param {*} sprintFolder путь до папки со спринтом по которому будет получены данные
 * @returns формат результата лучше смотреть в инструкции return
 */
function getRawSprintCommentData(dvarg, sprintFolder) {
    ...
    let rawData = listItemsFromAllComments
        .map(comment => {
            ...
            return {
                "user": page.file.frontmatter.user,
                "listItem": comment,
                "date": getDateFromFileName(page.file.name)
            }
        })
    console.log("rawdata:")
    console.log(rawData);
...
/**
 * Возвращает сырой массив данных по задачам, где каждая строка - карточка.
...
function getRawSprintTaskData(dv, sprintFolder) {
    ...
    const taskData = taskPages.map(page => {
        const listItems = page.file.inlinks.map(link => dv.page(link.path)).where(page => page.tags && dv.array(page.tags).includes("dailyComments")).flatMap(page => page.file.lists).where(listItem => listItem.cardref && listItem.cardref.path === page.file.path)
